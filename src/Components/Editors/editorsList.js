...
import { EnhancedTableToolbar } from "./editorsTableToolbar";
  ...
  useEffect(() => {
    try {
      ...
      const getStudents = async () => {
        try {
          const editorsSnapshot = await getDocs(collection(db, "editor"));
          const fetchedEditors = [];
          editorsSnapshot.forEach((editor) => {
            fetchedEditors.push({ docId: editor.id, ...editor.data() });
          });
          setRows(fetchedEditors);
          setIsLoading(false);
        } catch (err) {
          console.log("Error while fetching editors: " + err);
        }
      };
